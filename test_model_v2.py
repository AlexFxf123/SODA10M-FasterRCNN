#!/usr/bin/env python3
"""
Faster R-CNN v2 目标检测完整测试脚本
加载训练好的v2模型，对测试图片进行检测，并可视化结果
"""
...
def load_trained_model(model_path, device='cuda'):
    ...
    model = fasterrcnn_resnet50_fpn_v2(weights=None, min_size=1080, max_size=1920)  # 输入尺寸改为1920x1080
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='Faster R-CNN v2目标检测测试')
    parser.add_argument('--model', type=str, default='models_split_v2/faster_rcnn_best.pth',
                       help='训练好的v2模型路径 (默认: models_split_v2/faster_rcnn_best.pth)')
    parser.add_argument('--test_dir', type=str, default='test_images',
                       help='测试图片目录 (默认: test_images)')
    parser.add_argument('--output_dir', type=str, default='test_results_v2',
                       help='结果输出目录 (默认: test_results_v2)')
    ...
    print("="*80)
    print("Faster R-CNN v2 目标检测测试")
    print("="*80)
    print(f"v2模型文件: {args.model}")
    ...
    if not os.path.exists(args.model):
        print(f"错误: v2模型文件不存在: {args.model}")
        print(f"请确保v2模型文件存在，或使用 --model 参数指定正确的路径")
        return
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        print("使用示例参数运行...")
        print("您可以指定参数: python test_model_v2.py --model model.pth --test_dir images --output_dir results")
        print()
